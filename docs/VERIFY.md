# 验证结果（emulator-5558 / Android 14 API 34）

## 构建
- `assembleDebug`：成功，APK 见 `artifacts/local-voice-ime-debug.apk`。
- `testDebugUnitTest`：成功，CandidateEngine/状态等测试通过。
- `lintDebug`：成功，0 error。
- `connectedDebugAndroidTest`：成功，emulator-5558 上 3 个测试全部通过。

## 实测链路
1. `adb -s emulator-5558 install -r`.

...
当前正在推进第 2 板块“中文拼音 26 键”。开发分支：`fix/pinyin26-audit-round2`；PR：`#106`；本文创建时最新已知开发提交：`274ef7f1953f29bdd5c3e90e8f071e4fabfe0348`。该分支仍需以最新 GitHub HEAD/CI 为准，不要假设这里记录的 SHA 永远最新。
...
已完成：独立设置页与活动 IME 通过同一套 `applyPersistedSettings()` 一次应用完整持久化快照，避免主题、外观、皮肤参数分次重绘；`SETTINGS -> FUZZY_SETTINGS` 已有返回栈回归测试；密码字段的文本编辑面板会提前禁用粘贴并说明原因，避免点击后才失败。

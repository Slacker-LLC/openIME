...
当前正在推进第 11/12 板块的 UI/交互收敛。开发分支：`codex/uiux-redesign`；当前未创建 PR；最近的几何实现提交包括键盘圆角/尺寸、字体缩放联动、功能键文案和展开候选行适配，页面级统一提交随后补入本段。实时核对时远端 `origin/main` 无新增提交；具体领先数量以当前 `git rev-list --left-right --count origin/main...HEAD` 为准，后续仍需以最新远端 HEAD/CI 为准，不要假设这里记录的 SHA 永远最新。
...
已完成补强：松开空格只请求停止录音，不立即结束语音会话；后端排空最后一段 PCM、产生 partial/final 后才结束并自动上屏。新增了“松手后尾帧仍可到达最终结果”的交互回归，API 36 模拟器通过，真实手机回归已跑过该用例。
...
已完成补强：空格长按语音失败时在输入法窗口内使用危险色状态条，并将模型、麦克风权限和通用失败原因压缩为完整可读摘要；API 36 模拟器已真实长按空格验证，键盘高度与输入内容保持稳定。
...
已完成补强：原生键盘、候选栏、工具面板、设置页输入框和首页步骤胶囊统一使用语义几何尺度；键面 8dp、普通控制 12dp、卡片/输入框 16dp、步骤胶囊 28dp、全圆控件独立使用 pill 半径，并以 48dp 触控目标和 56dp 主行高作为尺寸基线。26 键、9 键、数字键共享网格行高、间距和双高 Enter 公式；字体放大时紧凑布局也沿用同一动态行高；功能键文案在大字体下采用受控缩小而非省略号；展开候选面板的按钮行也跟随动态行高。页面级设置/编辑 UI 也已收口：主设置输入卡片、编辑卡片、符号行统一 16dp，输入字段与主操作行统一 56dp，次级控件统一 12dp，弹窗保留 24dp 层级。API 36 模拟器已实际检查橙色强调色下的首页、偏好设置页、输入框焦点环、openIME 竖屏/横屏键盘窗口，并以 1.3 字体比例检查 9 键布局、“中/英”完整显示和展开候选面板，轮廓保持一致且无裁切。

已完成补强：语音波形、开关旋钮、长按符号选择器和删除手势提示等边缘控件也改用统一的 pill、键面和 badge 几何 token，避免主流程统一而细节控件各自硬编码半径。最新 APK 已重新安装到 API 36 模拟器，输入法窗口启动正常。

已完成补强：剪贴板/常用语面板中的新增、编辑、删除、刷新和浮动切换等动作在主题重建后仍保留明确的背景与按下反馈；“新增常用语”使用当前强调色作为主操作，编辑/删除等次级动作保持统一的 12dp 控件圆角。API 36 模拟器已沿真实输入法窗口路径检查剪贴板与常用语面板。

已完成补强：工具面板的说明、空状态与异常状态使用分层语义色；异常提示增加低饱和危险色背景，保证浅色/深色主题下都能被快速识别。API 36 模拟器已在浅色与深色系统主题下检查输入法窗口和首页状态卡片。

已完成补强：常用语和自定义符号编辑页的主操作在内容为空时使用真正的中性禁用态，不再保留容易误解为可点击的强调色背景；输入有效内容后恢复强调色，并通过 API 36 模拟器实机验证状态切换。
...
已完成：独立设置页与活动 IME 通过同一套 `applyPersistedSettings()` 一次应用完整持久化快照，避免主题、外观、皮肤参数分次重绘；`SETTINGS -> FUZZY_SETTINGS` 已有返回栈回归测试；密码字段的文本编辑面板会提前禁用全选、复制、剪切并说明原因，同时保留剪贴板入口和粘贴能力，普通编辑器还会依据实时选区和剪贴板内容提前禁用复制、剪切、粘贴，避免点击后才失败。
...
已完成补强：底部“中/英”按键保留紧凑视觉标签，但根据当前模式动态提供“切换到英文 26 键 / 中文 26 键 / 中文九键 / 文字键盘”的动作描述与当前模式状态描述，覆盖读屏和硬件键盘焦点场景；API 36 模拟器已实际完成中文 26 键、中文九键、英文 26 键和数字页之间的往返切换，回切会恢复用户偏好的中文布局。

...
当前正在推进第 11/12 板块的 UI/交互收敛。开发分支：`codex/uiux-redesign`；当前未创建 PR；最近的几何实现提交为 `2bc5e7f`（圆角）、`d2715fc`（尺寸）、`6cb1e5c`（字体缩放联动）和 `9d87e67`（功能键文案适配）。实时核对时远端 `origin/main` 无新增提交；具体领先数量以当前 `git rev-list --left-right --count origin/main...HEAD` 为准，后续仍需以最新远端 HEAD/CI 为准，不要假设这里记录的 SHA 永远最新。
...
已完成补强：原生键盘、候选栏、工具面板、设置页输入框和首页步骤胶囊统一使用语义几何尺度；键面 8dp、普通控制 12dp、卡片/输入框 16dp、步骤胶囊 28dp、全圆控件独立使用 pill 半径，并以 48dp 触控目标和 56dp 主行高作为尺寸基线。26 键、9 键、数字键共享网格行高、间距和双高 Enter 公式；字体放大时紧凑布局也沿用同一动态行高；功能键文案在大字体下采用受控缩小而非省略号。API 36 模拟器已实际检查橙色强调色下的首页、输入框焦点环、openIME 竖屏/横屏键盘窗口，并以 1.3 字体比例检查 9 键布局和“中/英”完整显示，轮廓保持一致且无裁切。
